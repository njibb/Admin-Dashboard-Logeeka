...
export default function ManajemenFaqPage() {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 401) {
          ...
          setErrorMsg("Gagal memuat data FAQ dari server.");
        }
      }
    } finally {
      setIsLoading(false); 
  ...
  const handleDelete = async (id: string) => {
    const confirmDelete = await Swal.fire({
      ...
      text: "Data FAQ ini tidak bisa dikembalikan!",
      icon: 'warning',
        ...
        <div className="flex items-center gap-4">
          ...
          {!isLoading && (
            <div className="flex items-center gap-1.5 px-3 py-1 bg-red-50 text-red-600 rounded-lg text-sm font-black border border-red-100 shadow-sm">
              ...
              {totalDataServer > 0 ? totalDataServer : faqData.length} Total Data
            </div>
          )}
        </div>

        <Link 
          href="/dashboardhome/faqs/tambah" 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/80">
                ...
                <th className="py-4 px-6 border-b border-gray-100 text-xs font-bold text-gray-500 uppercase tracking-wider">Pertanyaan (Judul)</th>
                <th className="py-4 px-6 border-b border-gray-100 text-xs font-bold text-gray-500 uppercase tracking-wider">Jawaban (Konten)</th>
                ...
                <tr>
                  <td colSpan={4} className="py-24 text-center">
                    <div className="flex flex-col items-center justify-center">
                      <h3 className="text-xl font-black text-gray-900 mb-2">Data Tidak Ditemukan! 💬</h3>
                      <p className="text-gray-500">Coba gunakan kata kunci lain atau tambahkan data baru.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={item.id} className="hover:bg-red-50/40 transition-colors group">
                   ...
                   <td className="py-4 px-6 border-b border-gray-100 text-sm text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <Link href={`/dashboardhome/faqs/edit/${item.id}`} title="Edit Portofolio" className="p-1.5 text-gray-400 hover:text-orange-500 hover:bg-orange-50 rounded-lg transition-all">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" /></svg>
                        </Link>
                        <button onClick={() => handleDelete(item.id)} title="Hapus Portofolio" className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all">
        ...
        <div className="p-4 border-t border-gray-100 flex items-center justify-between bg-gray-50">
          ...
          <div className="flex gap-2">
            ...
            <button 
              ...
              disabled={displayedFaq.length < entriesPerPage || isLoading}
              className={`px-4 py-2 text-sm font-bold rounded-lg transition-all ${displayedFaq.length < entriesPerPage || isLoading ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-100'}`}
            >
              Next
            </button>
          </div>
