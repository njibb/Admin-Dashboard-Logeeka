...
export default function ManajemenFaqPage() {
  ...
  const filteredFaq = faqData.filter((item: Faq) => {
    const searchLower = searchTerm.toLowerCase();
    const judulMatch = (item.judul || "").toLowerCase().includes(searchLower);
    const kontenMatch = (item.konten || "").toLowerCase().includes(searchLower);
    return judulMatch || kontenMatch;
  });

  const displayedFaq = filteredFaq.slice(0, entriesPerPage);
                ...
                <tr>
                  <td colSpan={4} className="py-24 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <h3 className="text-xl font-black text-gray-900 mb-2">Belum Ada FAQ Nih! 💬</h3>
                      <p className="text-gray-500 max-w-md mb-8 font-medium">Kamu belum menambahkan pertanyaan seputar layanan. Yuk bantu user dengan menambahkan FAQ!</p>
                      <Link 
                        href="/dashboardhome/faqs/tambah"
                ...
                displayedFaq.map((item: Faq) => (
                  <tr key={item.id} className="hover:bg-red-50/40 transition-colors group">
                    <td className="py-4 px-6 border-b border-gray-100 text-sm font-semibold text-gray-900 text-center">
                      {faqData.length - faqData.findIndex((p: Faq) => p.id === item.id)}
                    </td>
                    ...
                    <td className="py-4 px-6 border-b border-gray-100 text-sm text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <Link href={`/dashboardhome/faqs/edit/${item.id}`} title="Edit FAQ" className="p-1.5 text-gray-400 hover:text-orange-500 hover:bg-orange-50 rounded-lg transition-all">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" /></svg>
                        </Link>
                        <button onClick={() => handleDelete(item.id)} title="Hapus FAQ" className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all">
